	                ...
	                <c:forEach var="InquiryVO" items="${listInquiry}">
	                    <tr>
	                        ...
	                        <td>
	            				...
	            				<button class="btnModify" data-inquiryid="${InquiryVO.inquiryId}">답글 수정</button><!-- style="display:none;" 화면에서 감추는 코드 -->
	            				<!-- 수정: 클래스 이름을 btnDelete로 변경 -->
	            				<button class="btnDelete" data-inquiryid="${InquiryVO.inquiryId}">답글 삭제</button>
	                        </td>
	                    </tr>
	                    <tr>
	                        <td colspan="5">
	                            ...
	                            <div id="inputContainer_${InquiryVO.inquiryId}" class="inputContainer" style="text-align: center; margin-top: 10px; display: none;">
	                                ...
	                                <button class="btnSubmit" data-inquiryid="${InquiryVO.inquiryId}" data-productid="${InquiryVO.productId}">작성</button>
	                            </div>
	                            ...
	                            <div id="modifyContainer_${InquiryVO.inquiryId}" class="modifyContainer" style="text-align: center; margin-top: 10px; display: none;">
	                                ...
	                                <button class="btnModifySubmit" data-inquiryid="${InquiryVO.inquiryId}">수정</button>
	                            </div>
	                        </td>
	                    </tr>
	                </c:forEach>
	...
	$(document).ready(function(){
	    ...
	    $('.btnSubmit').click(function() {
	        let answerContent = $('#replyAnswer_' + $(this).data('inquiryid')).val();
	        let inquiryId = $(this).data('inquiryid');
	        let productId = $(this).data('productid');
	        //let productId = $('#inputContainer_' + inquiryId).find('.btnAdd').data('productid'); // 수정된 부분: productId 가져오기
	    ...
	    $('.btnModifySubmit').click(function() {
	    	let answerContent = $('#modifyAnswer_' + $(this).data('inquiryid')).val();
	    	let inquiryId = $(this).data('inquiryid');
